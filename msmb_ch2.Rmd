...
rootogram(gf1, xlab = "", rect_gp = gpar(fill = "chartreuse3"))
```
...
rootogram(GF2.1, xlab = "", rect_gp = gpar(fill = "chartreuse2"))
```
...
To obtain this value, we can use the **maximum likelihood estimator** (MLE), which by definition, "estimates the parameters of a probability distribution by maximizing a likelihood function, so that under the assumed statistical model the observed data is most probable" [(Wikipedia)](https://en.wikipedia.org/wiki/Maximum_likelihood_estimation).
...
```{r}
hap6 = read.table("./data/haplotype6.txt", header = TRUE, sep = "", dec = ".")
hap6
```
...
CpGtab = read.table("./data/model-based-cpg-islands-hg19.txt", header = TRUE) # Require addition function header = TRUE to load a text file
nrow(CpGtab)
head(CpGtab)
```
...
```{r}
set.seed(123456789)
E2.1 = rbinom(1000, prob = 0.0001, size = 1000)
E2.1
...
### 2.2 Make a function that generates n random uniform numbers between 0 and 7 and returns their maximum.  Then execute the function for n = 25 and repeat the procedure B = 100 times.  Plot the distribution.
...
set.seed(122)
RN(25)
fig2.2 = (replicate(100, RN(25)))
histogram(fig2.2)
```

What is the maximum likelihood estimate of the maximum of a sample of size 25?

...Not entirely sure how to do this, but seems similar to the example on page 23?
...
The variable was created....by dividing by 1344; however, not exactly sure of the significance of this number?
...
codonbias <- function(aa){
  ...
  #add observed frequencies of codon usage by factoring total counts
  aa_table <- mutate(aa_table, obs_freq = aa_table$Number / sum(aa_table$Number))
  
  #add expected frequency based on 1/number of codons
  aa_table <- mutate(aa_table, exp_freq = 1/nrow(aa_table))
  
  #find the difference between observed and expected frequencies
  aa_table <- mutate(aa_table, usage_diff = obs_freq - exp_freq)
  
  print(aa_table)
  row_max <- which.max(abs(aa_table$usage_diff))
  
  #required if the bias is AGAINST using a codon
  if(nrow(aa_table) > 2 && aa_table$usage_diff[row_max] < 0){
    print("Codon bias against, aka bias towards not using it")
    print(aa_table$Codon[row_max])
  }
  
  #requied if the bias is FOR using a codon
  else if (nrow(aa_table) > 2 && aa_table$usage_diff[row_max] > 0){
    print("Codon bias for, aka bias towards using it")
    print(aa_table$Codon[row_max])
  }
  
  #two codons availble means equal bias for/against
  else if (nrow(aa_table) == 2) {
    print("Only two available codons, usage for both codons deviates the same, equal bias")
    print(aa_table$Codon)
  }
  
  #one codon available means no bias possible
  else if (nrow(aa_table) == 1){
    print("Only one available codon")
  }
}
```
...
```{r}
codonbias2 <- function(){
  #prime with stop codon
  stop_table <- mtb %>% as_tibble %>% filter(mtb$AmAcid == "End")
  
  #finding observed frequencies for end
  stop_table <- mutate(stop_table, obs_freq = stop_table$Number / sum(stop_table$Number))

  #add expected frequency based on 1/number of codons
  stop_table <- mutate(stop_table, exp_freq = 1/nrow(stop_table))
  
  #find the difference between observed and expected frequencies
  stop_table <- mutate(stop_table, usage_diff = obs_freq - exp_freq)
  
  #aa vector
  aas <- c("Gly", "Glu", "Asp", "Val", "Ala", "Pro", "Leu", "Ile", "His", "Gln", "Arg", "Ser", "Phe", "Tyr", "Cys", "Thr", "Asn", "Lys", "Val")
  
  #add observed frequencies of codon usage by factoring total counts for all aas not stop
  for (query_aa in aas){
  each_aa_table <- mtb %>% as_tibble %>% filter(mtb$AmAcid == query_aa)
  
  #finding observed frequencies for aas
  each_aa_table <- mutate(each_aa_table, obs_freq = each_aa_table$Number / sum(each_aa_table$Number))

  #add expected frequency based on 1/number of codons
  each_aa_table <- mutate(each_aa_table, exp_freq = 1/nrow(each_aa_table))
  
  #find the difference between observed and expected frequencies
  each_aa_table <- mutate(each_aa_table, usage_diff = obs_freq - exp_freq)
  
  #add table back to aa_table
  stop_table <- full_join(stop_table, each_aa_table, by = c("AmAcid", "Codon", "Number", "PerThous", "obs_freq", "exp_freq", "usage_diff"))
  }
  
  stop_table %>%  print(n = Inf)
  row_max <- which.max(stop_table$usage_diff)
  print("Bias for")
  print(stop_table$Codon[row_max])
  row_min <- which.min(stop_table$usage_diff)
  print("Bias against")
  print(stop_table$Codon[row_min])
}
```
...
```
*Note*
- The beta distribution represents all the possible values of a probability when we don’t know what that probability is (i.e., a probability distribution of probabilities).


### 2.6 Re-analyse the data from Section 2.9.2 using a sketched prior.

Using the shiny app, https://jhubiostatistics.shinyapps.io/drawyourprior/, the prior were calculated to be $\alpha$ = 4, $\beta$ = 5
